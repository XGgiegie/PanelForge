...
export function createSeedanceVideoPrompt(input: SeedanceVideoPromptInput) {
  ...
  const sections: string[][] = [
    ...
    [`【情绪与动作】`, `围绕当前分镜制造清晰情绪变化，人物动作幅度适中，表情有细节，画面有电影感张力但不要混乱。`],
    [`【美术风格】`, `精致漫剧质感，干净构图，高级光影，主体突出，背景有层次，色彩统一，画面清晰细腻。`],
    extra ? [`【补充要求】`, extra] : [],
    [
      `【负向约束】`,
      `不要 logo，不要水印，不要无关文字，不要字幕乱入，不要多余肢体，不要脸部变形，不要角色换脸，不要服装突然变化，不要画面闪烁，不要低清晰度，不要复杂转场。`,
    ],
  ]
